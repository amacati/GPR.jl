...
exp1 = () -> doublependulum2D(nsteps, Δt=tsim, θstart=(rand(2).-0.5) .* [π, 2π])[1]
exp2 = () -> doublependulum2D(nsteps, Δt=tsim, θstart=[(rand()/2 + 0.5)*rand([-1,1]), 2(rand()-0.5)] .* π)[1]    # [-π:-π/2; π/2:π] [-π:π]
exptest = () -> doublependulum2D(nsteps, Δt=tsim, θstart=[π/2, 0])[1]
traindf, testdf = generate_dataframes(config, config["nsamples"], exp1, exp2, exptest)
...
storage = Storage{Float64}(200, 2)
mechanism
s, mechanism = doublependulum2D(1, Δt=0.01, θstart=[π/2, 0.0], ωstart=[0,0])[1:2]
